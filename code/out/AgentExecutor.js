...
class AgentExecutor {
    ...
    async run(userPrompt, chatHistory, model = 'local-model', signal, activeFile, thinking = true, geminiThinkingLevel = 'high', planningMode = false, attachedFiles, maxContextTokens = 16000) {
        ...
        const useNativeFunctionCalling = Boolean(provider.supportsNativeFunctionCalling?.() && provider.chatCompletionStreamWithTools);
        ...
        while (iteration < maxIterations) {
            ...
            if (useNativeFunctionCalling && provider.chatCompletionStreamWithTools) {
                ...
                response = await provider.chatCompletionStream(messages, model, this.temperature, (token) => {
                    this.onProgress({ type: 'token', output: token });
                }, signal, thinking, geminiThinkingLevel);
     ...
     */
    getSystemPrompt(nativeFunctionCalling = false) {
