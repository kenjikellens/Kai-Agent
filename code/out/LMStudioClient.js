...
class LMStudioClient {
     ...
     */
    async chatCompletion(messages, model = 'local-model', temperature = 0.7, signal) {
     ...
     */
    async chatCompletionStream(messages, model = 'local-model', temperature = 0.7, onToken, signal, thinking = true) {
        const isGemini = model && model.toLowerCase().startsWith('gemini');
        if (isGemini) {
     ...
     */
    _chatFreeProvider(messages, model, temperature, provider) {
        return new Promise((resolve, reject) => {
     ...
     */
    _chatFreeProviderStream(messages, model, temperature, onToken, signal, provider) {
        return new Promise((resolve, reject) => {
            const apiKey = this._getProviderApiKey(provider);
            if (!apiKey) {
                reject(new Error(`No API key configured for ${provider.name}. Add it in Settings.`));
     ...
     */
    async chatGemini(messages, model, temperature, signal) {
        return new Promise((resolve, reject) => {
     ...
     */
    async chatGeminiStream(messages, model, temperature, onToken, signal, thinking = true) {
        return new Promise((resolve, reject) => {
            const apiKey = this.apiKey;
            if (!apiKey) {
                reject(new Error('Gemini API key is not configured in settings. Please add your API key.'));
