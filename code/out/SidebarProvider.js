...
class SidebarProvider {
    ...
    resolveWebviewView(webviewView, _context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            switch (data.type) {
                case 'sendMessage': {
                    await this._handleSendMessage(data.messages, data.model, data.thinking, data.geminiThinkingLevel || 'high', data.planningMode || false);
                    break;
                }
                case 'abort': {
                    if (this._activeAbortController) {
     ...
     */
    async _handleSendMessage(messages, model, thinking = true, geminiThinkingLevel = 'high', planningMode = false) {
        if (!this._view) {
            return;
        }
        ...
        try {
            ...
            const runResult = await executor.run(userPrompt, chatHistoryWithoutLast, model || 'local-model', this._activeAbortController.signal, activeFile, thinking, geminiThinkingLevel, planningMode);
