...
/**
 * Manages active language resolution and translation lookup for KAI Agent Standalone.
 */
export class I18nManager {
    ...
    /**
     * Resolves the active language code based on setting or system locale.
     */
    public static getActiveLanguage(): string {
        const config = ConfigManager.getInstance().getConfig();
        const setting = config.language || 'auto';
        if (setting === 'auto') {
            const sysLocale = (app?.getLocale ? app.getLocale() : Intl.DateTimeFormat().resolvedOptions().locale || 'en').toLowerCase();
            if (LOCALES[sysLocale]) return sysLocale;
            const baseLang = sysLocale.slice(0, 2);
